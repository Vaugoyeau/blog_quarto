...
Et là, petite subtilité, il faut le préciser dans les paramétrages du chunck, grâce à `context: server`. 
Dans ce chunck, je créé aussi une table de vérification que va afficher que quelques lignes de `data()` pour vérifier que ça fonctionne grâce à la fonction `renderTable()` du package `{shiny}`.  
